...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.85\textwidth]{TXcorner_crop.jpg}
...
\paragraph{Lines}
Lines are the most ambiguous landmarks among all the landmarks in the field, because a single line can be matched to any field line in the field. Thus ,the lines also can not be used for global localization if features are not extracted from them. To find the line correspondence, a similar methodolgy as the nearest neighbor search is adopted. 


%To cope with this, the same methodolgy from junction correspondence is taken, namely local position optimization. 
