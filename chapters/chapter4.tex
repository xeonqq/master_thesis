...
\begin{tikzpicture}[
  ...
  \matrix {
    \node[source] (motion) {Motion Module};
      \& \node[source] (vision) {Vision Model};  \\
...
$Sc$ is a scaling matrix and $Sc^2$ is square elementwise. It intuitively means, when the robot walks \SI{1}{\meter} in x or y direction, it may have an error range of $\pm$ \SI{0.8}{\meter}. When it rotates \SI{1}{\radian}, it may have an error of $\pm$ \SI{0.8}{\radian} in orientation. The value of $Sc$ depends on the accuracy of odometry measurement from the motion module, and a better tuning of the values can be argued.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{fieldLcorners.png}
...
\begin{itemize}
  ...
  \item resolution limitation of image
\end{itemize}
...
\begin{tikzpicture}
    ...
    \end{scope}
\end{tikzpicture}
\caption[Landmarks in image plane]{Landmarks in image plane. The red box indicates the lines which detected from the front line of the penalty area.}
...
\begin{figure}[h!]
  \centering
\begin{tikzpicture}
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{measure_err_clean.png}};
    \begin{scope}[x={(image.south east)},y={(image.north west)}]
        \draw[red,ultra thick, dashed, rounded corners] (0.841,0.62) rectangle (0.918,0.284);
    \end{scope}
\end{tikzpicture}
\caption[Landmarks in robot frame]{Landmarks in robot frame. Grey lines indicate the detected lines after projection, the red box indicates the lines which detected from the front line of the penalty area.}
